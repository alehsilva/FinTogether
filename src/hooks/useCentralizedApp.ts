...
export function useCentralizedApp() {
  ...
  const getDefaultCategory = useCallback(
    (type: 'receita' | 'despesa') => {
      return categories.find(
        cat => cat.name === 'Sem Categoria' && cat.type === type && cat.is_system === true
      );
    },
    [categories]
  );
  ...
  const handleUpdateTransaction = useCallback(
    async (params: { transactionId: string; status: string }) => {
  ...
  const handleEditTransaction = useCallback(
    async (transactionId: string, data: any) => {
