...
export class TransactionService {
  ...
  private static mapSpecialType(type: 'simples' | 'parcela' | 'assinado'): 'simples' | 'parcela' | 'assinado' {
  ...
  private static async ensureUserExists(userId: string) {
    try {
      ...
      const { data, error } = await supabase
        .rpc('ensure_user_exists', { user_uuid: userId })
  ...
  }  private static applyOptionalFilters(query: any, filters: TransactionFilters = {}, options?: { includePrivacy?: boolean }) {
    if (filters.type) query = query.eq('type', filters.type)
    if (options?.includePrivacy && filters.privacy) query = query.eq('privacy', filters.privacy)
    if (filters.category_ids && filters.category_ids.length > 0) query = query.in('category_id', filters.category_ids)
    if (filters.account_ids && filters.account_ids.length > 0) query = query.in('account_id', filters.account_ids)
    ...
    if (filters.search_term) query = query.or(`title.ilike.%${filters.search_term}%,description.ilike.%${filters.search_term}%`)
    return query
  }
  ...
  static async getTransactions(
    ...
    if (selectedView === 'casal') {
      ...
      if (coupleId) {
        ...
        query = query.or(`couple_id.eq.${coupleId},and(user_id.eq.${userId},couple_id.is.null)`) as any
      } else {
