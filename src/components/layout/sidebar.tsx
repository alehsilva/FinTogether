...
export function Sidebar({ user, className }: SidebarComponentProps) {
  ...
  const renderNavItem = (item: NavItem) => {
    ...
    return (
      <Button
        ...
        size="icon"
        className={cn(
          'w-12 h-12 rounded-xl transition-all duration-150 cursor-pointer',
          isActive
  ...
  return (
    <div
      className={cn(
        'flex h-full w-[72px] flex-col bg-white/95 dark:bg-slate-800/95 backdrop-blur-xl border-r border-slate-200/80 dark:border-slate-700/50',
        className
      )}
    >
      {/* Header */}
      <div className="flex h-16 items-center justify-center border-b border-slate-200/80 dark:border-slate-700/50"></div>

      {/* Navegação principal - apenas ícones */}
      <ScrollArea className="flex-1 py-4">
        <div className="flex flex-col items-center space-y-3 px-3">
          {/* Menu principal */}
          <div className="space-y-3">{navigation.map(item => renderNavItem(item))}</div>
        </div>
      </ScrollArea>

      {/* Footer */}
      <div className="border-t border-slate-200 dark:border-slate-700/50 p-3"></div>
    </div>
  );
}
