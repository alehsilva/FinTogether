        ...
        return (
            <div className="space-y-1.5">
                ...
                <div className="relative group">
                    {leftIcon && (
                        <div className="absolute left-3 top-1/2 transform -translate-y-1/2 text-slate-400 group-focus-within:text-emerald-400 transition-colors">
                            {leftIcon}
                        </div>
                    )}
                    ...
                    <input
                        ref={ref}
                        className={`
              w-full px-3 py-2 border rounded-lg transition-all duration-200 bg-slate-800/60 text-slate-100 backdrop-blur-md shadow-sm text-sm
              ${leftIcon ? 'pl-10' : ''}
              ${rightIcon ? 'pr-10' : ''}
              ${hasError
            ...
            `}
                    ...
                    {rightIcon && (
                        <div className="absolute right-3 top-1/2 transform -translate-y-1/2 text-slate-400 group-focus-within:text-emerald-400 transition-colors">
                            {rightIcon}
                        </div>
                    )}
...
export function PasswordInput({
    ...
    return (
        <div className="space-y-1.5">
            <label className="text-xs font-medium text-slate-700 dark:text-slate-300 tracking-wide transition-colors duration-300">
                {label}
    ...
    ({ label, error, helperText, options, placeholder, className, required, ...props }, ref) => {
        ...
        return (
            <div className="space-y-1.5">
                ...
                <div className="relative group">
                    <select
                        ref={ref}
                        className={`
              w-full px-3 py-2 border rounded-lg transition-all duration-200 appearance-none bg-slate-800/60 text-slate-100 backdrop-blur-md shadow-sm text-sm
              ${hasError
                                ? 'border-red-500/50 bg-red-900/20 focus:border-red-500 focus:ring-2 focus:ring-red-500/20'
                                : 'border-slate-700/40 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 hover:border-slate-600/60'
                            }
            ...
            `}
                    ...
                    <div className="absolute right-3 top-1/2 transform -translate-y-1/2 pointer-events-none group-focus-within:text-emerald-400 transition-colors">
            ...
            <div className="space-y-1.5">
                ...
                <div className="relative group">
                    <textarea
                        ...
                        maxLength={maxLength}
                        className={`
              w-full px-3 py-2 border rounded-lg transition-all duration-200 resize-vertical min-h-[80px] bg-slate-800/60 text-slate-100 backdrop-blur-md shadow-sm text-sm
              ${hasError
                                ? 'border-red-500/50 bg-red-900/20 focus:border-red-500 focus:ring-2 focus:ring-red-500/20'
                                : 'border-slate-700/40 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 hover:border-slate-600/60'
                            }
            ...
            `}
                ...
                <div className="flex justify-between items-center">
                    ...
                    {showCharCount && maxLength && (
                        <div className={`text-sm font-medium ${charCount > maxLength * 0.9 ? 'text-orange-400' : 'text-slate-500'}`}>
                            {charCount}/{maxLength}
                        </div>
                    )}
                </div>
