...
export function AddTransactionPanel({
    ...
    const [selectedTransactionType, setSelectedTransactionType] = useState<'simples' | 'parcela' | 'assinado'>('simples')
    ...
    const getDefaultCategoryId = () => {
        const semCategoria = categories.find(c =>
    ...
    useEffect(() => {
        if (editingTransaction && categories.length > 0) {
            const tipoCorreto = editingTransaction.type === 'receita' || editingTransaction.type === 'despesa'
            ...
            let categoriaValue = editingTransaction.category_id ||
                (editingTransaction as any).category?.id ||
                (typeof (editingTransaction as any).category === 'string' ? (editingTransaction as any).category : '');
            ...
            if (categoryExists) {
                ...
                const formData = {
                    valor: editingTransaction.amount ? String(editingTransaction.amount).replace('.', ',') : '',
                    ...
                    transaction_date: editingTransaction.transaction_date ? editingTransaction.transaction_date.split('T')[0] : getLocalDateString(),
    ...
    }, [editingTransaction?.id, lastEditingTransactionId, hasCouple, reset, defaultType, categories, activeTab]);
    ...
    const containerClasses = isFixed
        ? `hidden lg:block w-80 flex-shrink-0 transition-all duration-500 relative ${editingTransaction
            ? 'ring-2 ring-emerald-500/30 shadow-2xl shadow-emerald-500/20 z-20 bg-white dark:bg-slate-900 border-l-2 border-emerald-500'
            : 'z-10 bg-white dark:bg-slate-900 backdrop-blur-xl border-l border-slate-200 dark:border-slate-700/30'
        }`
    ...
    return (
        <div ref={panelRef} className={containerClasses}>
            <div className={`${contentClasses} flex flex-col`}>
                <div className={`flex items-center justify-between p-3 lg:p-3 border-b transition-all duration-300 relative z-30 flex-shrink-0 ${editingTransaction && isFixed
                    ...
                    <div className="flex items-center gap-2.5 relative z-40">
                        <div className={`w-1 h-6 rounded-full ${editingTransaction ? 'bg-gradient-to-b from-emerald-400 to-emerald-600' : 'bg-gradient-to-b from-slate-400 to-slate-500 dark:from-slate-500 dark:to-slate-600'}`} />
                        <div>
                            <h2 className="text-sm font-bold text-slate-900 dark:text-slate-50 tracking-tight transition-colors duration-300">
                                {editingTransaction ? 'Editar Transação' : 'Nova Transação'}
                            </h2>
                            {editingTransaction && isFixed && (
                                <div className="text-[10px] text-emerald-600 dark:text-emerald-400 flex items-center gap-1 font-medium mt-0.5 transition-colors duration-300">
                    ...
                    {(!isFixed || editingTransaction) && (
                        <button
                            onClick={handleClose}
                            className="p-1.5 hover:bg-slate-200 dark:hover:bg-slate-700/60 rounded-lg transition-all duration-200 relative z-40 cursor-pointer group"
                            title={isFixed ? "Cancelar edição" : "Fechar"}
                        >
                            <X className="w-4 h-4 text-slate-500 dark:text-slate-400 group-hover:text-slate-900 dark:group-hover:text-slate-100 group-hover:rotate-90 transition-all duration-200" />
                        </button>
                    )}
                </div>
                ...
                <div className="flex-1 overflow-y-auto">
                    <form onSubmit={handleSubmit(onSubmit)} className="p-3 lg:p-3 pb-8 space-y-3 w-full overflow-x-hidden">
                        ...
                        <div className="mt-4 pt-3 border-t border-slate-300/30 dark:border-slate-700/30 transition-colors duration-300">
                            <Button
                                type="submit"
                                disabled={isLoading || !isValid}
                                className={`w-full py-3 px-4 rounded-lg font-bold text-sm text-white transition-all duration-300 shadow-lg relative overflow-hidden group ${effectiveTab === 'despesa'
