...
export function TransactionList({
  ...
  return (
    <>
      <Card
        className={`flex-1 overflow-hidden border-slate-200 dark:border-slate-500/30 bg-gradient-to-br from-slate-50/90 to-white/70 dark:from-slate-800/50 dark:to-slate-800/50 backdrop-blur-xl shadow-lg hover:shadow-xl transition-all duration-300 group ${isMaximized ? 'rounded-none shadow-lg -mx-4 px-4' : 'md:max-h-none max-h-[500px]'
      ...
      >
        ...
        <div className="px-3 pb-2 pt-3 border-b border-slate-300/40 dark:border-slate-700/30 transition-colors duration-150">
          ...
          <div className="flex items-center justify-between gap-1 md:gap-2 w-full">
            ...
            <div
              className={`flex items-center bg-slate-200/50 dark:bg-slate-700/30 rounded-full flex-shrink min-w-0 transition-all duration-200 md:ml-0 xl:ml-2
      ${showSearchInput ? 'hidden sm:flex md:scale-100' : 'flex md:scale-100'}
    `}
            >
              <button
                onClick={() => setTypeFilter('all')}
                className={`filter-button px-2.5 sm:px-3 md:px-4 py-1 sm:py-1.5 md:py-2 text-xs sm:text-sm md:text-base font-medium rounded-full transition-all duration-150 flex items-center
        ${typeFilter === 'all'
        ...
        ${typeFilter === 'despesa'
            ...
            <div className="relative flex-shrink-0">
              {showSearchInput ? (
                <div className="hidden sm:flex items-center">
                  <div className="relative">
                    <Input
                      ...
                      autoFocus
                      style={{ fontSize: '16px' }}
                      className="pr-8 pl-3 py-1 h-8 w-36 text-xs border-slate-300 dark:border-slate-600 focus:border-emerald-500 dark:focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 transition-all"
                    />
                    ...
                    <button
                      ...
                      className="absolute right-2 top-1/2 -translate-y-1/2 text-slate-500 dark:text-slate-400 hover:text-emerald-600 dark:hover:text-emerald-400 transition"
                    >
                    ...
                    {searchTerm && (
                      <button
                        ...
                        className="absolute right-8 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 transition"
                        title="Limpar busca"
                      >
                        ✕
                      </button>
                  ...
                  <button
                    onClick={toggleSearch}
                    className="ml-1 p-2 text-emerald-600 dark:text-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-950/50 rounded-lg active:scale-95"
                    title="Fechar busca"
                  >
                    ✕
                  </button>
                ...
                <button
                  onClick={toggleSearch}
                  className="p-2 text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700/50 rounded-xl active:scale-95"
        ...
        <CardContent
          ...
          className={`space-y-2 px-4 pb-4 pt-3 overflow-y-auto ${isMaximized ? 'h-[calc(100vh-380px)]' : 'max-h-[400px] md:max-h-[600px] lg:max-h-[500px]'
            ...
            filteredTransactions.map((transaction: Transaction, index: number) => {
              ...
              return (
                ...
                >
                  ...
                  <div
                    className={`relative w-full flex items-center gap-2 md:gap-2.5 p-2 md:p-2.5 rounded-lg transition-transform duration-150 cursor-pointer border ${isPending
                  ...
                  >
                    <div className="flex items-center gap-2 md:gap-2.5 flex-1 min-w-0">
                      <div
                        className={`w-7 h-7 md:w-8 md:h-8 rounded-full flex items-center justify-center shadow-sm flex-shrink-0 ${transaction.type === 'receita'
                      ...
                      >
                        {transaction.type === 'receita' ? (
                          ...
                          <svg
                            className="w-3.5 h-3.5 md:w-4 md:h-4 text-rose-400"
                            fill="none"
                            stroke="currentColor"
                            viewBox="0 0 24 24"
                          >
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="font-semibold text-xs md:text-sm truncate text-slate-900 dark:text-slate-100 transition-colors duration-150">
                          {transaction.title}
                        </p>
                        <div className="flex items-center gap-1.5 flex-wrap">
                          <p className="text-[10px] md:text-xs text-slate-700 dark:text-slate-400 truncate transition-colors duration-150">
                          ...
                          {isPending && (
                            <Badge
                              variant="outline"
                              className="text-[10px] md:text-xs px-1.5 py-0 border-amber-500/50 text-amber-600 dark:text-amber-400 flex items-center gap-1 flex-shrink-0 transition-colors duration-150"
                            >
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      <div className="text-right">
                        <p
                          className={`font-bold text-xs md:text-sm ${isPending ? 'opacity-75' : ''} ${transaction.type === 'receita'
                        ...
                        <p className="text-[10px] md:text-xs text-slate-600 dark:text-slate-500 font-medium transition-colors duration-150">
                      ...
                      <button
                        ...
                        disabled={loadingTransactions.has(transaction.id)}
                        className={`p-3 md:p-2 rounded-full transition-colors duration-150 disabled:opacity-50 disabled:cursor-not-allowed flex-shrink-0 cursor-pointer ${isPending
